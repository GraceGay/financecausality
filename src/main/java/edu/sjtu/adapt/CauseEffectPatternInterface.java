...
//					"因而" ,"只怪" ,"这说明" ,"表明了","这表明" ,"出于" ,"起源于" ,,"的原因" ,"原因是" ,"取决于" ,"导致" )
			put(Pattern.compile("([\\s\\S]*?)(因为|由于|出于|源于)([\\s\\S]*?)(，[\\s\\S]*)?(，?所以|导致[\\s]?了?|因此|故)([\\s\\S]+?)，([\\s\\S]+)"), 136);
			put(Pattern.compile("([\\s\\S]+)，([\\s\\S]+?)(所以|导致[\\s]?了?|因此|致使)([\\s\\S]+?)，([\\s\\S]+)"), 224);
			put(Pattern.compile("([\\s\\S]+)，([\\s\\S]+?)(是[\\s]?因为|是[\\s]?由于|原因是)([\\s\\S]+?)，([\\s\\S]+)"), 342);
